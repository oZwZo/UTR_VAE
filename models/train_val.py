    ...
    for idx,data in enumerate(dataloader):
        ...
        if idx % int(loader_len/5) == 0:
            if 'VAE' in popen.model_type:
                ...
                train_verbose = "{:5d} / {:5d} ({:.1f}%): \t LOSS:{:.9f} \t lr: {:.9f} ".format(idx,loader_len,idx/loader_len*100,
                                                                                                    loss.item(),
                                                                                                    lr)
...
def validate(dataloader,model,popen,epoch):
    ...
    with torch.no_grad():
        for idx,data in enumerate(dataloader):
            ...
            elif popen.dataset in ['MTL','ribo']:
    ...
    if "VAE" in popen.model_type:
        ...
        Total_loss = loss_dict['Total'].cpu().numpy().item()
        val_verbose = "\t LOSS:{:.7f}  Avg_ACC: {}".format(Total_loss,avg_acc)
